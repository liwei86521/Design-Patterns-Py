...
class infoServer(Server): # infoServer --> RealSubject
    def recv(self,info):
        self.content=info
        return "recv OK!"
    def send(self,info):
...
# 接收的数据格式必须如info_struct所示，服务器仅接受info_struct的content字段。那么，如何给这个服务器设置一个
# 白名单，使得只有白名单里的地址可以访问服务器呢？修改Server结构是个方法，但这显然不符合软件设计原则中的单一职责原则。
...
#代理中有一个server字段，控制代理的服务器对象，infoServerProxy充当Server的直接接口代理，而whiteInfoServerProxy直接
# 继承了infoServerProxy对象，同时加入了white_list和对白名单的操作。这样，在场景中通过对白名单代理的访问，
# 就可以实现服务器的白名单访问了。
def test1():
    info_struct = dict()
    ...
    info_server = infoServer() # 初始化 RealSubject 类
